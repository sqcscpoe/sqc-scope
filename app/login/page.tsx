...
if(res?.ok){router.push('/dashboard')}else{setError('Email o contraseña incorrectos');setLoading(false)}}
return(<div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--dark)',backgroundImage:'radial-gradient(ellipse 80% 60% at 50% -10%, rgba(200,152,10,0.12) 0%, transparent 60%)'}}>
...
style={{width:'100%',padding:'10px 14px',background:'var(--dark3)',border:'1px solid var(--border)',borderRadius:'var(--radius)',color:'var(--text)',fontFamily:'var(--font-body)',fontSize:14,outline:'none'}}/></div>
<div><label style={{display:'block',fontSize:12,color:'var(--text-dim)',marginBottom:6,fontWeight:500}}>CONTRASEÑA</label>
...
{loading?'Entrando...':'Entrar'}</button></form></div>
