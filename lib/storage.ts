export interface Flag {
  severity: 'DISQUALIFYING' | 'REJECTION' | 'WARNING'; ruleId: string; category: string
  description: string; page: string; action: string
}
export interface Project {
  id: string; company: string; userId: string; projectName: string; system: string
  roofType: string; result: 'APROBADO'|'APROBADO_CON_WARNINGS'|'RECHAZADO'|'DESCALIFICADO'|'EN_PROGRESO'
  flags: Flag[]; summary: string; totalDQ: number; totalRejection: number; totalWarning: number
  createdAt: string; updatedAt: string; pdfName?: string
}
const store: Map<string, Project[]> = new Map()
export function getProjects(company: string): Project[] {
  return (store.get(company) || []).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
}
...
function extractBetween(text: string, start: string, end: string): string | null {
  const si = text.indexOf(start); if (si === -1) return null
  const from = si + start.length; const ei = text.indexOf(end, from)
...
export function parseReport(raw: string, projectName: string, company: string, userId: string, pdfName: string): Project {
  const id = Math.random().toString(36).slice(2); const now = new Date().toISOString()
  ...
  const flags: Flag[] = flagsRaw.split('\n').filter(l=>l.trim()&&l.includes('|')).map(line => {
    const p = line.split('|')
    return { severity: (p[0]?.trim()||'WARNING') as Flag['severity'], ruleId: p[1]?.trim()||'',
      category: p[2]?.trim()||'', description: p[3]?.trim()||'', page: p[4]?.trim()||'', action: p[5]?.trim()||'' }
  })
  return { id, company, userId, projectName, system, roofType, result, flags, summary,
    totalDQ: dq, totalRejection: rejection, totalWarning: warning, createdAt: now, updatedAt: now, pdfName }
}